...
/*interrupt config*/
	if (gpio_is_valid(hall_info->irq_gpio)) {
		...
		rc = devm_request_threaded_irq(&pdev->dev, hall_info->irq, NULL, hall_interrupt, IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING|IRQF_ONESHOT, "hall-switch-irq", hall_info);
		if (rc < 0) {
